use crate::{
    errors::MappedErrors,
    traits::{Is32bitsProcess, MapMemory, ToU8Vec},
    utils::{bytes2string, prepare_process_entry},
    win_funcs::{
        create_tool_help_32_snapshot, get_module_base_name_a, is_syswow_64, open_process,
        process_32_next, read_process_memory, virtual_query_ex,
    },
};
...
use winapi::um::{
    ...
    winnt::{
        HANDLE, MEMORY_BASIC_INFORMATION, MEM_COMMIT, PAGE_READONLY, PAGE_READWRITE,
        PROCESS_ALL_ACCESS,
    },
};
...
impl Process {
    pub fn by_pid(process_id: u32) -> Result<Self, MappedErrors> {
        ...
        Ok(Process {
            ...
            architecture: if process_handle.get_architecture()? {
                32
            } else {
                64
            },
