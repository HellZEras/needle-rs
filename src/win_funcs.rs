...
use winapi::{
    shared::{
        basetsd::SIZE_T,
        minwindef::{BOOL, DWORD, FARPROC, HMODULE, LPCVOID, LPDWORD, LPVOID, PUSHORT},
        ntdef::LPSTR,
    },
    um::{
        libloaderapi::{GetModuleHandleA, GetProcAddress},
        memoryapi::{ReadProcessMemory, VirtualAllocEx, VirtualQueryEx, WriteProcessMemory},
        ...
        tlhelp32::{CreateToolhelp32Snapshot, Process32Next, LPPROCESSENTRY32},
        winnt::{HANDLE, LPCSTR, PMEMORY_BASIC_INFORMATION},
...
pub fn write_process_memory(
    ...
    let func_return = unsafe {
        WriteProcessMemory(
            hprocess,
            lp_baseaddress,
            lp_buffer,
            nsize,
            lp_number_of_bytes_written.unwrap_or(null_mut()),
        )
    };
    if func_return == 0 {
...
pub fn get_module_handle_a(lp_module_name: LPCSTR) -> Result<HMODULE, MappedErrors> {
...
pub fn get_proc_address(h_module: HMODULE, lp_proc_name: LPCSTR) -> Result<FARPROC, MappedErrors> {
...
pub fn process_32_next(h_snapshot: HANDLE, lppe: LPPROCESSENTRY32) -> Result<bool, MappedErrors> {
...
pub fn is_syswow_64(
    ...
    let func_result: i32 = unsafe {
        IsWow64Process2(
            h_process,
            p_process_machine,
            p_native_machine.unwrap_or(null_mut()),
        )
    };
    if func_result == 0 {
...
pub fn read_process_memory(
    ...
    unsafe {
        ReadProcessMemory(
            ...
            lp_buffer,
            n_size,
            lp_number_of_bytes_read.unwrap_or(null_mut()),
        ) != 0
    }
}
